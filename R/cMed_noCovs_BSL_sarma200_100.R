...
{
  log(theta[1] > 0 & theta[2] > 0 & theta[2] < 1 & theta[2]+theta[3]<1 &
...
sim <- function(theta, T)
{
  ...
  x <- theta[1]+sarima.sim(ar=c(theta[2], theta[3]), sar=theta[4], S=12,
...
pr <- tryCatch(arima(x, order=c(2,0,0), seasonal=list(order=c(1,0,0), period=12)))
...
sp_bsl_glasso_cMed ### Best penalty parameter: lambda = 0.00442
...
resultMigrant <- bsl(y = as.numeric(dCMRsmo), n = 200, M = 50000, model = model, 
                   ...
                   method = 'BSL', shrinkage="glasso", penalty=0.00442, parallel=FALSE, verbose=TRUE)
...
est_distr <- "../Results/estad_distr_CMR_sarma200_100.pdf"
...
posterior_probs <- lapply(resultMigrant@theta[, 9], FUN=function(x){EM(dCMRsmo, 2, 1e-16, x)})
...
### Export the results
name_data <- "../Results/CMR_incidence_sarma200_100.csv"
...
save(list=ls(), file="../Results/cMed_results_sarma200_100.RData")
