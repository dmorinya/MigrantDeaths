...
### AR(1)
ar1 <- arima(dWMRsmo, order=c(1,0,0))
garch  <- garch(ar1$residuals, order=c(0, 1))
AIC_AR1 <- AIC(garch)

### AR(2)
ar2 <- arima(dWMRsmo, order=c(2,0,0))
garch  <- garch(ar2$residuals, order=c(0, 1))
AIC_AR2 <- AIC(garch)

### ARMA(1, 1)
arma11 <- arima(dWMRsmo, order=c(1,0,1))
garch  <- garch(arma11$residuals, order=c(0, 1))
AIC_ARMA11 <- AIC(garch)

### SAR(1)
sar1 <- arima(dWMRsmo, order=c(1,0,0), seasonal=list(order=c(1,0,0), period=12))
garch  <- garch(sar1$residuals, order=c(0, 1))
AIC_SAR1 <- AIC(garch)

### SAR(2)
sar2 <- arima(dWMRsmo, order=c(2,0,0), seasonal=list(order=c(1,0,0), period=12))
garch  <- garch(sar2$residuals, order=c(0, 1))
AIC_SAR2 <- AIC(garch)

### SARMA(1, 1)
sarma11 <- arima(dWMRsmo, order=c(1,0,1), seasonal=list(order=c(1,0,0), period=12))
