...
sp_bsl_glasso_eMed <- selectPenalty(ssy = ssy, n = 200,
                                    lambda_all, theta = init_vals, M = 100, sigma = 10.5, model = model,
                                    method = 'BSL', shrinkage = 'glasso')
sp_bsl_glasso_eMed ### Best penalty parameter: lambda = 0.00656
...
est_distr <- "../Results/estad_distr_EMR_sarma100_100.pdf"
...
posterior_probs <- lapply(resultMigrant@theta[, 8], FUN=function(x){EM(dEMRsmo, 2, 1e-16, x)})
...
### Export the results
name_data <- "../Results/EMR_incidence_sarma100_100.csv"
write.csv(pr, name_data, row.names=FALSE)

plot(as.numeric(df3$deaths_EMR), type="l", ylim=c(0, 250))
lines(as.numeric(colMeans(pr)), col="red")

sum(dEMRsmo)/sum(colMeans(pr))*100
sum(df3$deaths_EMR)/sum(colMeans(pr))*100
save(list=ls(), file="../Results/eMed_results_sarma100_100.RData")
