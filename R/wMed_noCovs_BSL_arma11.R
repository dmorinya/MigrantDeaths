...
dWMRsmo<-ts(c(unlist(kf$Xs)),start=c(2009,1),freq=12)
xlim1<-c(2009, 2021)
...
logPrior <- function(theta)
{
  log(theta[1] > 0 & theta[2] > 0 & theta[2] < 1 & theta[5] > 0 & theta[5] < 1 &
        theta[4] > 0 & theta[5] > 0 & theta[5] < 2 & theta[7] > 0 &
...
sim <- function(theta, T)
{
  ...
  error <- vector()
  error[1] <- 0
  error <- ARCH1(T, theta[4], theta[5])
  x <- theta[1]+arima.sim(model=list(order=c(1,0,1), ar=theta[2], ma=theta[3]),
                          rand.gen=function(n, ...) rnorm(n, mean=0, sd=theta[7]), n=T)+error
  z  <- rbinom(T, 1, theta[6])
  q <- theta[8]
  y  <- x*(1-z)+q*z*x
  y[y<0] <- 0
  return(y)
...
init <- normalmixEM(dWMRsmo, maxit=10000)
...
x <- ifelse(ind==1, dWMRsmo/q_init, dWMRsmo)
pr <- tryCatch(arima(x, order=c(1,0,1)))
init_ar1  <- tryCatch((garch(!is.na(pr$resid), order=c(0, 1))$coef))

init_vals <- c(mean(x), pr$coef[1], pr$coef[2], init_ar1, init$lambda[init$mu==min(init$mu)],
               init_sigma, q_init)

library(quantspec)
model <- newModel(fnSim = sim, fnSum = st,
                  simArgs = list(T = length(dWMRsmo)), theta0 = init_vals,
                  fnLogPrior = logPrior, thetaNames=c(expression(phi[0]), expression(alpha[1]), expression(beta[1]), "intercept",
...
sp_bsl_glasso_cMed ### Best penalty parameter: lambda = 0.0214
...
est_distr <- "../Results/estad_distr_WMR_arma11.pdf"
...
posterior_probs <- lapply(resultMigrant@theta[, 8], FUN=function(x){EM(dWMRsmo, 2, 1e-16, x)})
ur_indicator    <- lapply(posterior_probs, FUN=function(x){ifelse(x$gamma[, 1] > x$gamma[, 2], 1, 0)})
q <- resultMigrant@theta[, 8]
rec_values <- list()
for (j in 1:length(ur_indicator))
{
  den <- q[j]
  rec_values[[j]] <- ifelse(ur_indicator[[j]]==0, dWMRsmo, 
                            dWMRsmo/den)
}
...
### Export the results
name_data <- "../Results/WMR_incidence_arma11.csv"
write.csv(pr, name_data, row.names=FALSE)

plot(as.numeric(df3$deaths_WMR), type="l", ylim=c(0, 1200))
lines(as.numeric(colMeans(pr)), col="red")

sum(dWMRsmo)/sum(colMeans(pr))*100
sum(df3$deaths_WMR)/sum(colMeans(pr))*100
save(list=ls(), file="../Results/wMed_results_arma11.RData")
