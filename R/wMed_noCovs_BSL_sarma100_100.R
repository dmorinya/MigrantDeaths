...
sim <- function(theta, T)
{
  ...
  x <- theta[1]+sarima.sim(ar=theta[2], sar=theta[3], S=12,
...
pr <- tryCatch(arima(x, order=c(1,0,0), seasonal = list(order = c(1,0,0), period=12)))
...
model <- newModel(fnSim = sim, fnSum = st,
                  ...
                  fnLogPrior = logPrior, thetaNames=c(expression(phi[0]), expression(alpha[1]), expression(alpha[2]), "intercept", 
                                                      expression(gamma[1]), expression(omega),  expression(sigma), "q"))
...
est_distr <- "../Results/estad_distr_WMR_sarma100_100.pdf"
...
### Export the results
name_data <- "../Results/WMR_incidence_sarma100_100.csv"
write.csv(pr, name_data, row.names=FALSE)

plot(as.numeric(df3$deaths_WMR), type="l", ylim=c(0, 250))
lines(as.numeric(colMeans(pr)), col="red")

sum(dWMRsmo)/sum(colMeans(pr))*100
sum(df3$deaths_WMR)/sum(colMeans(pr))*100
save(list=ls(), file="../Results/wMed_results_sarma100_100.RData")
