...
df_daily_med_global <- df_daily_med %>% group_by(date) %>% summarise(deaths=sum(total_dead_and_missing), survivors=sum(total_survivors))
...
df_daily_med_global <- df_daily_med_global %>% 
  mutate(date_month = ymd(paste0(year(date),"-",month(date),"-",01))) %>% 
  group_by(date_month) %>% 
  mutate(share_deaths_day = deaths/sum(deaths)) # used to compute HHI (fractionalization)
...
df_monthly_med_global <- df_daily_med_global %>% 
  mutate(date_month = ymd(paste0(year(date),"-",month(date),"-",01))) %>% 
  group_by(date_month) %>% 
  summarise(total_dead_and_missing = sum(deaths, na.rm=T),
            total_survivors = ifelse(date_month < min(df_mmp_daily_med$date),NA,sum(survivors, na.rm = T)),
            hhi_deaths_month = sum(share_deaths_day^2)) %>% # HHI (fractionalization index)
  ungroup() %>% 
  rename(date = date_month) %>% 
  select(date,total_dead_and_missing,total_survivors,hhi_deaths_month)

# Join the frontex data on arrivals
df_frontex_monthly_med_global <- df_frontex_monthly_med %>% group_by(date) %>%
  summarise(total_arrivals_frontex=sum(total_arrivals_frontex))
df_monthly_med <- left_join(df_monthly_med, df_frontex_monthly_med, by=c("date","route"))
df_monthly_med_global <- left_join(df_monthly_med_global, df_frontex_monthly_med_global, by=c("date"))

df_monthly_med_global <- df_monthly_med_global[!duplicated(df_monthly_med_global), ]
df_monthly_med <- df_monthly_med[!duplicated(df_monthly_med), ]
