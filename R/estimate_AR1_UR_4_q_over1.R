...
ar1_model <- function()
{
  ...
  for (t in 1:N)
  {
    ...
    mu[2, t] <- q_b0*(delta_b0)/(1-phi)
    tau.m[1, t] <- (sigma2_w/(1-phi^2))^-1
    tau.m[2, t] <- (1/(q_b0^2))*(sigma2_w/(1-phi^2))^-1
...
params <- c("w_b0", "q_b0", "delta_b0", "phi", "sigma2_w", "component_chosen")
...
stopCluster(cl)
save(list="cmed_route", file="../Results/cmed_route_AR1_4_q_over1.RData")
