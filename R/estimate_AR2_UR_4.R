...
ar2_model <- function()
{
  ...
  for (t in 1:N)
  {
    logit(q[t]) <- q_b0
    logit(omega[t]) <- w_b0
    mu[1, t] <- (delta_b0)/(1-phi1-phi2)
    mu[2, t] <- q[t]*(delta_b0)/(1-phi1-phi2)
    tau.m[1, t] <- (sigma2_w/(1-phi1^2-phi2^2))^-1
    tau.m[2, t] <- (1/(q[t]^2))*(sigma2_w/(1-phi1^2-phi2^2))^-1
...
data_CMR <- list(y=df2$Incidence_CMR, N=length(df2$Incidence_CMR))

params <- c("w_b0", "q_b0", "delta_b0", "phi1", "phi2", "sigma2_w", "component_chosen")
...
system.time(cmed_route <- jags.parallel(data = data_CMR, inits = NULL, parameters.to.save = params,                 model.file = ar2_model, n.chains = 5, n.iter = 50000, n.burnin = 10000, n.thin = 1000, 
            ...
            jags.module = "mix"))
stopCluster(cl)
save(list="cmed_route", file="Results/cmed_route_covsAR2_4.RData")