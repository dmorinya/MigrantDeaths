...
init <- normalmixEM(dCMRsmo, maxit=10000)
...
x <- ifelse(ind==1, dCMRsmo/q_init, dCMRsmo)
...
model <- newModel(fnSim = sim, fnSum = st,
                  simArgs = list(T = length(dCMRsmo)), theta0 = init_vals,
...
### Estimating the SL
set.seed(1234)
resultMigrant <- bsl(y = as.numeric(dCMRsmo), n = 200, M = 50000, model = model, 
...
est_distr <- "../Results/estad_distr_CMR_arma11.pdf"
...
posterior_probs <- lapply(resultMigrant@theta[, 8], FUN=function(x){EM(dCMRsmo, 2, 1e-16, x)})
...
{
  den <- q[j]
  rec_values[[j]] <- ifelse(ur_indicator[[j]]==0, dCMRsmo, 
                            dCMRsmo/den)
}
...
### Export the results
name_data <- "../Results/CMR_incidence_arma11.csv"
write.csv(pr, name_data, row.names=FALSE)

plot(as.numeric(df3$deaths_CMR), type="l", ylim=c(0, 1200))
lines(as.numeric(colMeans(pr)), col="red")

sum(dCMRsmo)/sum(colMeans(pr))*100
sum(df3$deaths_CMR)/sum(colMeans(pr))*100
save(list=ls(), file="../Results/cMed_results_arma11.RData")
