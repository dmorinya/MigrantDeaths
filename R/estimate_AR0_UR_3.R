...
ar0_model <- function()
{
  ...
  for (t in 1:N)
  {
    ...
    mu[1, t] <- delta_b0
    mu[2, t] <- q[t]*delta_b0
    tau.m[1, t] <- sigma2_w^-1
    tau.m[2, t] <- (1/(q[t]^2))*sigma2_w^-1
...
params <- c("w_b0", "w_b1", "q_b0", "delta_b0", "sigma2_w", "component_chosen")
...
stopCluster(cl)
save(list="cmed_route", file="../Results/cmed_route_AR0_3.RData")
