...
sim <- function(theta, T)
{
  ...
  x <- theta[1]+arima.sim(model=list(order=c(2,0,0), ar=c(theta[2], theta[3])),
...
pr <- tryCatch(arima(x, order=c(2,0,0)))
...
est_distr <- "../Results/estad_distr_CMR_arma200.pdf"
...
### Export the results
name_data <- "../Results/CMR_incidence_arma200.csv"
...
save(list=ls(), file="../Results/cMed_results_arma200.RData")
