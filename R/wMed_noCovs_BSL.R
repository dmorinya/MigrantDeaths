...
sim <- function(theta, T)
{
  ...
  x <- theta[1]+arima.sim(model=list(order=c(1,0,0), ar=theta[2]), 
                          rand.gen=function(n, ...) rnorm(n, mean=0, sd=theta[6]), n=T)+error
...
resultMigrant <- bsl(y = as.numeric(dWMRsmo), n = 200, M = 50000, model = model, 
                   diag(c(.0005^2,.0005^2,.0005^2,.0005^2,.0005^2,.0005^2,.0005^2)),
...
est_distr <- "../Results/estad_distr_WMR.pdf"
...
posterior_probs <- lapply(resultMigrant@theta[, 7], FUN=function(x){EM(dWMRsmo, 2, 1e-16, x)})
ur_indicator    <- lapply(posterior_probs, FUN=function(x){ifelse(x$gamma[, 1] > x$gamma[, 2], 1, 0)})
q <- resultMigrant@theta[, 7]
rec_values <- list()
...
### Export the results
name_data <- "../Results/WMR_incidence.csv"
...
save(list=ls(), file="../Results/wMed_results.RData")
